...
class AudioManager:
    ...
    def _resolve_assets_path(self, assets_path: str | Path | None) -> Path:
        """
    ...
    def play_audio_file(self, filename: str) -> bool:
        ...
        if not self._available:
            logger.debug(
                "Audio playback skipped - system not available", filename=filename
            )
            return False
        ...
        try:
            ...
            logger.error(
                "Could not start audio playback thread", filename=filename, error=str(e)
            )
            return False
    ...
    def _play_audio_thread(self, filename: str) -> None:
        ...
        try:
            ...
            logger.error("Error during audio playback", filename=filename, error=str(e))
