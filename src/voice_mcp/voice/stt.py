...
class TranscriptionHandler:
    ...
    def create_recorder(
        self,
        on_realtime_transcription_callback: Callable[[str], None] | None = None,
        on_recording_stop_callback: Callable[[str], None] | None = None,
        ...
        if config.stt_server_mode:
            server_recorder = self._create_server_recorder(
                on_realtime_transcription_callback, on_recording_stop_callback, language
            )
            if server_recorder:
    ...
    def _create_server_recorder(
        ...
        try:
            ...
            if not recorder:
                logger.warning(
                    "Failed to get recorder from STT server", model=self.model_name
                )
                return None
    ...
    def _create_oneoff_recorder(
        ...
        try:
            ...
            if on_realtime_transcription_callback:
                recorder_config["on_realtime_transcription_stabilized"] = (
