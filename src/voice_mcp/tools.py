...
from .voice.text_output import TextOutputController, OutputMode
from .voice.hotkey import HotkeyManager
from .voice.stt_server import STTServerManager, create_stt_server, ensure_stt_server, get_stt_server
...
def _server_transcribe_once(recorder: AudioToTextRecorder, duration: Optional[float] = None, language: str = "en") -> Dict[str, Any]:
    """
    ...
    logger.debug("Starting server-based transcription", duration=duration, language=language)
    ...
    try:
        ...
        recorder.set_on_recording_start_callback(lambda: logger.debug("Recording started"))
...
class VoiceTools:
    ...
    @staticmethod
    def speak(text: str, voice: Optional[str] = None, rate: Optional[int] = None, volume: Optional[float] = None) -> str:
        """
    ...
    def listen(
        ...
        try:
            ...
            if server is not None and server._is_running:
                ...
                if recorder is None:
                    logger.warning("STT server recorder not available, falling back to one-off transcription")
                    use_server = False
            ...
            if use_server:
                ...
                result = _server_transcribe_once(recorder, duration=duration, language=language)
            else:
            ...
            if output_mode != "return" and transcribed_text:
                ...
                if not output_result["success"]:
                    logger.warning("Text output failed", error=output_result.get("error"))
    ...
    def start_hotkey_monitoring() -> str:
        ...
        try:
            ...
            if result["success"]:
                logger.info("Hotkey monitoring started via tools", hotkey=config.hotkey_name)
